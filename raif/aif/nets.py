...
Description: This is the neural networks API. Adapted from Danija Hafner's code.
  This file also implement Contrastive Recurrent State Prior Preference model
  (nested inside RSSM)
"""
...
# CRISPY RSSM
# Contrastive Recurrent State Prior Preference inside Recurrent State Space Model
class CRSPP_RSSM(nj.Module):
  ...
  def observe_no_action(self, embed, is_first, state=None):
    swap = lambda x: x.transpose([1, 0] + list(range(2, len(x.shape))))
    if state is None:
      state = self.initial_all_zeros(embed.shape[0])
    step = lambda prev, inputs: self.obs_step_no_action(prev[0], *inputs)
    inputs = swap(embed), swap(is_first)
    start = state, state
    post, prior = jaxutils.scan(step, inputs, start, self._unroll)
    post = {k: swap(v) for k, v in post.items()}
    prior = {k: swap(v) for k, v in prior.items()}
  ...
  def imagine_no_action(self, bs, horizon, state=None):
    state = self.initial_all_zeros(bs) if state is None else state
    assert isinstance(state, dict), state
    _state = state.copy()
    all_states = []
    for step in range(horizon):
      _state = self.img_step_no_action(_state)
      all_states.append(_state.copy())
    all_states = {k: jnp.stack([all_states[i][k] for i in range(horizon)], 1) for k in all_states[0].keys()}
    return all_states
  ...
  def obs_step_no_action(self, prev_state, embed, is_first):
    is_first = cast(is_first)
    prev_state = jax.tree_util.tree_map(
        lambda x: self._mask(x, 1.0 - is_first), prev_state)
    prev_state = jax.tree_util.tree_map(
        lambda x, y: x + self._mask(y, is_first),
        prev_state, self.initial(len(is_first)))
    prior = self.img_step_no_action(prev_state)
    x = jnp.concatenate([prior['deter'], embed], -1)
    x = self.get('obs_out_2', Linear, **self._kw)(x)
    stats = self._stats('obs_stats_2', x)
    dist = self.get_dist(stats)
    stoch = dist.sample(seed=nj.rng())
    post = {'stoch': stoch, 'deter': prior['deter'], **stats}
    return cast(post), cast(prior)
  ...
  def img_step(self, prev_state, prev_action):
    ...
    x = self.get('img_in', Linear, **self._kw)(x, dropout=True)
    x, deter = self._gru(x, prev_state['deter'])
    x = self.get('img_out', Linear, **self._kw)(x, dropout=True)
  ...
  def img_step_no_action(self, prev_state):
    prev_stoch = prev_state['stoch']
    if self._classes:
      shape = prev_stoch.shape[:-2] + (self._stoch * self._classes,)
      prev_stoch = prev_stoch.reshape(shape)
    x = self.get('img_in_2', Linear, **self._kw)(prev_stoch)
    x, deter = self._gru_2(x, prev_state['deter'])
    x = self.get('img_out_2', Linear, **self._kw)(x)
    stats = self._stats('img_stats_2', x)
    dist = self.get_dist(stats)
    stoch = dist.sample(seed=nj.rng())
    prior = {'stoch': stoch, 'deter': deter, **stats}
    return cast(prior)
  ...
  def _gru_2(self, x, deter):
    x = jnp.concatenate([deter, x], -1)
    kw = {**self._kw, 'act': 'none', 'units': 3 * self._deter}
    x = self.get('gru_2', Linear, **kw)(x)
    reset, cand, update = jnp.split(x, 3, -1)
    reset = jax.nn.sigmoid(reset)
    cand = jnp.tanh(reset * cand)
    update = jax.nn.sigmoid(update - 1)
...
class ImageEncoderResnet(nj.Module):
  ...
  def __call__(self, x):
    ...
    depth = self._depth
    x = (jaxutils.cast_to_compute(x) - 0.5) * 2
    # print(x.shape)
...
class Linear(nj.Module):
  ...
  def __init__(
      self, units, act='none', norm='none', bias=True, dropout=0.0, outscale=1.0,
  ...
  def __call__(self, x, dropout=False):
    ...
    if self._bias:
      ...
      x += bias
    if dropout:
      if self._dropout == 1: x = jnp.zeros_like(x)
      mask = jax.random.uniform(nj.rng(), x.shape) > self._dropout
      x = jnp.asarray(mask, dtype=jnp.float32) * x / (1.0 - self._dropout)
