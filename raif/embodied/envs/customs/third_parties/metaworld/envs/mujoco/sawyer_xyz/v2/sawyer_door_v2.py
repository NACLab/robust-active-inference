...
class SawyerDoorEnvV2(SawyerXYZEnv):
    def __init__(self, tasks=None, render_mode=None, **kwargs): # NOTE: added kwargs
    ...
    def _reward_pos(obs, theta):
        ...
        # NOTE: Changed here for numpy 1.25 deprecation warning
        opened = a * np.asarray(theta < -np.pi / 90.0).astype(np.float32) + b * reward_utils.tolerance(
