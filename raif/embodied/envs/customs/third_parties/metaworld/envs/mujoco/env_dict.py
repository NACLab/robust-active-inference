...
def create_hidden_goal_envs():
    ...
    for env_name, env_cls in ALL_V2_ENVIRONMENTS.items():
        ...
        def initialize(env, seed=None, **kwargs): # NOTE: Added this
            if seed is not None:
                st0 = np.random.get_state()
                np.random.seed(seed)
            super(type(env), env).__init__(**kwargs) # NOTE: Added this
...
def create_observable_goal_envs():
    ...
    for env_name, env_cls in ALL_V2_ENVIRONMENTS.items():
        ...
        def initialize(env, seed=None, render_mode=None, **kwargs): # NOTE: Added this
            if seed is not None:
                st0 = np.random.get_state()
                np.random.seed(seed)
            super(type(env), env).__init__(**kwargs) # NOTE: Added this
